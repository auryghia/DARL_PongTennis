...
from tensorflow.keras.models import Model, load_model  # USA tensorflow.keras
...
from tensorflow.keras.optimizers import Adam, RMSprop  # USA tensorflow.keras
from tensorflow.keras import backend as K  # USA tensorflow.keras
import cv2
...
def OurModel(input_shape, action_space, lr):
    ...
    action = Dense(action_space, activation="softmax", kernel_initializer="he_uniform")(
        X
    )
...
class A2CAgent:
    ...
    def __init__(self, env, env_name="Pong-v0"):
        # Initialization
    ...
    def replay(self):
        ...
        actor_history = self.Actor.fit(
            states, actions, sample_weight=advantages, epochs=1, verbose=0
        )
        critic_history = self.Critic.fit(states, discounted_r, epochs=1, verbose=0)

        # Get the loss values
        actor_loss = actor_history.history["loss"][0]
        critic_loss = critic_history.history["loss"][0]

        # Store losses to be written with episode data
        self.current_actor_loss = actor_loss
        self.current_critic_loss = critic_loss
        self.states, self.actions, self.rewards = [], [], []
    ...
    def PlotModel(self, score, episode):
        ...
        current_average = sum(self.scores[-50:]) / len(self.scores[-50:])
        self.average.append(current_average)
        if str(episode)[-2:] == "00":  # much faster than episode % 100
            ...
            try:
                ...
                pass
        actor_loss_to_log = getattr(self, "current_actor_loss", "N/A")
        critic_loss_to_log = getattr(self, "current_critic_loss", "N/A")
        with open(self.log_file_path, "a") as f:
            f.write(
                f"{episode},{score},{current_average:.2f},{actor_loss_to_log},{critic_loss_to_log}\n"
            )

        # Reset current losses for the next episode
        if hasattr(self, "current_actor_loss"):
            del self.current_actor_loss
        if hasattr(self, "current_critic_loss"):
            del self.current_critic_loss
    ...
    def GetImage(self, frame):
        ...
        if frame_cropped.shape[0] != self.COLS or frame_cropped.shape[1] != self.ROWS:
            ...
            frame_cropped = cv2.resize(
                frame, (self.COLS, self.ROWS), interpolation=cv2.INTER_CUBIC
            )

        # converting to RGB (numpy way)
        frame_rgb = (
            0.299 * frame_cropped[:, :, 0]
            + 0.587 * frame_cropped[:, :, 1]
            + 0.114 * frame_cropped[:, :, 2]
        )
    ...
    def step(self, action):
        ...
        next_state = self.GetImage(next_obs)
